package com.dragon.talon.algorithm.copy;
...
public class Tree23<T extends Comparable<T>> {
    ...
    private class Node {
         ...
         * 重新平衡树的最深处和第二深处。
         * <p>
         * 算法试图在每个子元素中放入一个元素，但在一个关键的情况下，我们必须平衡
         * 从较高级别删除当前级别的树。
         */
        ...
        private void rebalance() {
            ...
            while (!isBalanced()) {
                ...
                if (getLeftSon().getLeftElement() == null) { // The unbalance is in the left child
                    ...
                    //临界情况下，最深层次的每个节点(子节点)只有一个元素(右边是空的)
                    //算法必须从树的更高层次重新平衡
                    if (getRightElement() == null) {
                    ...
                    /**
                     在这种情况下，我们可以有两种情况:
                     *
                     *(1)中间的子元素是满的，所以我们必须把元素向右移动
                     *
                     *(2)中间的子元素只有左边的元素，然后我们必须把右边的元素
                     *作为中间子节点的右元素
                     */
